...
export async function updateAddress(req, res) {
    try {
        ...
        const user = req.user;
        const address = user.addresses.id(addresId);
        if (!address) {
        ...
        // if the deault is not equel to undefined we are going to get that value but if its undefined we are going to live it has its.
        address.isDefault = isDefault !== undefined ? isDefault : address.isDefault;

        await user.save()

        res.status(201).json({ message: "Address Updated successfully", addresses: user.addresses })
...
export async function addToWishlist(req, res) {
    try {
        ...
        // check if the product is aleady in the wishlist
        if(user.wishlist.includes(productId)){
            return res.status(400).json({error: "Product already in wishlist"});
        }
        // if(user.wishlist.includes(productId)){
        //     return user.wishlist.pull
        // } 
